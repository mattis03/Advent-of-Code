...
class RuleBook2 {
    ...
    public RuleBook2(LinkedList<String> rules, LinkedList<String> pages) {
        for (String r : rules) {
    ...
    private void reOrderPage(LinkedList<Integer> page) {
        while (!pageIsValid(page)) {
            for (int i = 0; i < page.size() - 1; i++) {
                for (int j = i + 1; j < page.size(); j++) {
                    int n = page.get(i);
                    int k = page.get(j);
                    LinkedList<Integer> kRules = RULES.get(k);
                    // Byt plats på n och k
